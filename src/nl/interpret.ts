...
function systemPrompt(ctx: InterpretContext, cal: Calendar): string {
  ...
  return [
    ...
    "- To delete a transaction outright, use remove_transaction with a selector and",
    "  nothing else. 'delete the last charge', 'remove the $12 coffee', 'the Starbucks",
    "  one was never mine, take it off' → remove_transaction. Deleting cannot be undone,",
    "  so choose it only when the user wants the record GONE. If they want the spend",
    ...
    "  'none' for period/window/selector_kind. date stays \"\" unless window is 'day'.",
    ...
    "- For ONE named calendar date, use window 'day' and put that date in the date",
    "  field as YYYY-MM-DD. 'show history of 2026-07-19', 'what did I spend on July 19',",
    "  'yesterday's charges' → list_transactions, window 'day', date '2026-07-19'.",
    "  Resolve relative wording ('yesterday', 'last Friday', 'the 19th') against today's",
    "  date above and write the absolute date — the date field is the ONLY way to name a",
    "  single day, and period_offset counts whole weeks/months, never days. If you cannot",
    "  work out which date they mean, return 'unknown' and ask; do not widen the request",
    "  to the surrounding week.",
    "- A single day never uses report — use list_transactions. There is nothing to",
    "  aggregate over one day that the list of charges does not already show.",
    "- window 'all' means every charge ever recorded, with no date limit. Use it when",
    "  the user asks for a whole history rather than a period: 'show all of my gift",
    "  spending', 'everything I've spent on gifts', 'my full gift history' → window",
    "  'all'. Do not answer one of these with a week — an unbounded question answered",
    "  with a recent window reads as 'you spent nothing'.",
    "- scope decides WHICH ENVELOPES count, never which dates. 'main' is the main",
    "  weekly budget on its own and is the right default — money filed into a named",
    "  envelope is not weekly spending. Use 'category' with a category name when they",
    "  ask about one envelope, and 'all' only when they explicitly want every envelope",
    "  together.",
    "- The word 'all' in a message is about TIME far more often than about envelopes.",
    "  'show all of my gift spending' names one envelope and asks for its whole",
    "  history: scope 'category' with category 'gift', window 'all'. It is NOT scope",
    "  'all' — that would drop the gift filter and list unrelated spending back. Reach",
    "  for scope 'all' only when no envelope is named ('show everything', 'all my",
    "  spending across every budget').",
    "",
    "Multiple actions:",
